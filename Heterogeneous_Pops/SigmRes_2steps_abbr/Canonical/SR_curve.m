...
% Considering HETEROGENEITY IN individual agents' kinetic rate constant.
...
k_r_sdev = 0.025;                      % mean basal reverse rate (1st order)
...
avg_k_r = zeros(s,t_steps+1);       % For storing instantaneous <k_r(t)>
...
for n=1:s
    ...
    avg_k_r(n,1) = tempRp_log * k_r' / sum(tempRp_log); 
    ...
    while t <= t_steps
        ...
        tempRp_log = Rpv(2,:)==1;     % Logical array of existing Rp agents at end of time step
        avg_k_r(n,t+1) = tempRp_log * k_r' / sum(tempRp_log);    % Find <k_r> at end of time step
...
for n=1:s
    ...
    Rp_ss(n) = agents * S^2 / ( k_r_mean/k_f + S^2 );
    ssr(n) = k_r_mean * Rp_ss(n);
    % ssr: Steady-state rate of forward and back reactions
    
    dRpdt_synt(n,:) = k_f * S^2 .* (agents - Rp_array);  
