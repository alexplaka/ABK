...
% Here, we consider the case where only some agents X are sensitive to a delay 
% (i.e., distinct subspecies). The delay value is constant.
...
% Preallocate memory
X_all = zeros(reps,t_steps+1);         Xd_all = zeros(reps,t_steps+1);         
...
%   Markov process, so only previous and current time steps needed --> 2 rows:    
    ...
    % Array for storage of binary value indicating if specified X agent is sensitive
    % to a delay in the feedback reaction. (0: no delay; 1: delay)
    X_delay = zeros(1,size(Xv,2));      
...
% ABK simulation
    ...
    while t <= t_steps % && Tep(t)<agents
        ...
        if rand < P_b            
            ...
            % Now determine if newly created X agent is sensitive to a delay in feedback rx.
            X_delay(1,tempXd(chooseX1)) = round(rand); % Assume 50% chance of sensitivity
        ...
        if rand < P_s
            ...
            % Now determine if newly created X agent is sensitive to a delay in feedback rx.
            X_delay(1,tempXd(chooseX2)) = round(rand); % Assume 50% chance of sensitivity     
        end
    ...
    %     end
        ...
        for i = 1:size(tempX,2)
            ...
            if t <= to/dt || X_delay(1,tempX(i)) == 0
                P_d2(t) = k_d2 * Trp(t) * dt;                % P wrt each X molecule
            else            % set delay of value lag_mean
                P_d2(t) = k_d2 * Trp(t - lag_mean /dt) * dt;           
            end                        
            % ****************************************************************************  
        ...
        Tx(t+1) = sum(Xv(2,:));             
        Txd(t+1) = sum(X_delay);            % sum of delay-sensitive X agents  
        Tr(t+1) = sum(Rv(2,:));             
        Trp(t+1) = sum(Rpv(2,:));      
    ...
    X_all(n,:) = Tx;                        Xd_all(n,:) = Txd;       
    Rp_all(n,:) = Trp;
        
end             % *** end 'for n=1:reps' loop ***

clear Tx Txd Tr Trp Xv Rv Rpv temp* i j k n t chooseX* X_delay;
...
avg_Xd = mean(Xd_all);                  sdev_Xd = std(Xd_all);          cv_Xd = sdev_Xd ./ avg_Xd;   
...
plot(time(1:20:end),Xd_all(trial,1:20:end),'k'); 
...
title(['$\textbf{Subspecies R1 delay: } \delta =' num2str(lag_mean) ...
    '\, \textrm{sec}$'],'Interpreter','LaTeX');

leg0 = legend('$\textrm{ABK } N_R(t)$','$\textrm{ABK } N_{R1}(t)$',...
    '$\textrm{ABK } N_{Xp}(t)$',...
    '$\textrm{DDE } N_R(t)$','$\textrm{DDE } N_{Xp}(t)$',...
    'Location','NorthEast');
...
p_avgXd = plot(time(1:20:end),avg_Xd(1:20:end),'k','LineWidth',2,...
    'DisplayName','$\textrm{ABK } < \! N_{R1}(t) \! >$');
% plot(time(1:20:end),avg_Xd(1:20:end)+sdev_Xd(1:20:end),'LineStyle','--','Color',[0.8 0.8 0.8]);
% plot(time(1:20:end),avg_Xd(1:20:end)-sdev_Xd(1:20:end),'LineStyle','--','Color',[0.8 0.8 0.8]);
...
leg1 = legend([p_avgX , p_avgXd , p_avgRp , p_ddeX , p_ddeRp]);
