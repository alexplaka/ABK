...
for a=1:reps
    ...
    %  ABK Simulation
        ...
        while t*dt <= maxTime               % Additional condition: abs(Tr(t)-R_ss) > 0.5
            ...
            P_f(t) = k_f * S / (Km_f + Tr(n,t)) * dt;  % P_dif forward rx
            P_r(t) = k_r / (Km_r + Trp(n,t,a)) * dt;   % P_dif reverse rx
...
for n=1:s
    ...
    dRpdt_synt(n,:) = k_f * S .* (agents - Rp_array) ./ (Km_f + (agents - Rp_array));  

    [Rp_ss_int(n), ssr(n)] = intersections(Rp_array,dRpdt_degr,Rp_array,dRpdt_synt(n,:));
    ...
    if mod(n,5) == 0
        p(n) = plot(Rp_array,dRpdt_synt(n,:),'-','Color',[0 0.5 0],'DisplayName','Synthesis');
        plot(Rp_ss_int(n),ssr(n),'ok');
    end
end

p_d = plot(Rp_array,dRpdt_degr,'-m','DisplayName','Degradation');
hold off;

xlabel('N_R');        ylabel('dN_R/dt');            % For report 
% xlabel('R_p');        ylabel('dR_p/dt');    
set(gca,'XMinorTick','on','YMinorTick','on','Box','off');
leg0 = legend([p(5) p_d],'Location','NorthEast');
set(leg0,'FontName','Times New Roman','FontSize',9,...
    'EdgeColor',[0.95 0.95 0.95]);

% Create textbox
annotation(fig0,'textbox',...
    [0.15 0.132 0.111332007952286 0.0519953917050691],...
    'String',{'N_S = 5'},...
...
% Create textbox
annotation(fig0,'textbox',...
    [0.15 0.48 0.121153081510935 0.0519953917050691],...
    'String',{'N_S = 30'},...
    'FontWeight','demi','FontName','Times New Roman',...
    'FitBoxToText','off','LineStyle','none','Color',[0 0.75 0]);
...
% Create textbox
annotation(fig0,'textbox',...
    [0.15 0.86 0.121153081510935 0.0519953917050691],'String',...
    {'N_S = 60'},...
    'FontWeight','demi','FontName','Times New Roman',...
    'FitBoxToText','off','LineStyle','none','Color',[0 0.75 0]);
...
plot(S_array,ABK_Rp+ABK_Rp_dev,'--','Color',[0.8 0.8 0.8]);
plot(S_array,ABK_Rp-ABK_Rp_dev,'--','Color',[0.8 0.8 0.8]);
...
% p2.mainLine.DisplayName = 'ABK';

xlabel('N_S');                              % ylabel('N_{Rp}^*');  
ylabel('N_R^*');                            % For Report    
axis([0 S_array(end) 0 agents]);                        hold off;
set(gca,'XMinorTick','on','YMinorTick','on','Box','off');
leg1 = legend('DE','ABK <N_R^*>');
% leg1 = legend([p1 p2.mainline.DisplayName]);
...
% - k_f * S and k_r should be within a factor of two so that the steep rise 
% of the sigmoid curve occurs approx. in the middle of range of S values (all
% else being equal).