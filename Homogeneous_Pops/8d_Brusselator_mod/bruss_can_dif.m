...
dydt(1) = b - 2 * k_f * X^2 + 2 * k_r * Z + a * Z * Y  - (c + d) * X;        
dydt(2) = c * X - a * Z * Y;
dydt(3) = k_f * X^2 - k_r * Z;
...
% 
% dydt(1) = b + a * (k_f / k_r) * X^2 * Y - (c + d) * X;        
% dydt(2) = c * X - a * (k_f / k_r) * X^2 * Y;
% 
% In other words, the system reduces to the unmodified Brusselator with
% the rate constant for the trimolecular rx being a * (k_f / k_r).
