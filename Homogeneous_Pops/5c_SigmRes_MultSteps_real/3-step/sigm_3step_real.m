...
Trppp = zeros(1,t_steps);             % Total sum of Rppp agents in each time step
...
while t*dt <= maxTime
    ...
    Trpp(t+1) = sum(Rppv(2,:));             Trppp(t+1) = sum(Rpppv(2,:));
...
clear Rv Rpv Rppv Rpppv;
finaltime = (t-1) * dt;

%% Solve differential equation (was waiting for variable 'finaltime')
[t_sol, y_sol] = ode45(@sigm_3step_real_dif,0:finaltime/100:finaltime+(finaltime/20),...
    [Tr(1) ; Trp(1) ; Trpp(1) ; Trppp(1)]);
...
disp(['Diff eq terminal Rppp value = ' num2str(y_sol(end,4))]);
...
scatter(time,Trpp,3,'oy');
scatter(time,Trppp,3,'ok');

scatter(t_sol,y_sol(:,1),3,'b');                  
scatter(t_sol,y_sol(:,2),3,'r');  
scatter(t_sol,y_sol(:,3),3,'c');                               
scatter(t_sol,y_sol(:,4),3,'g');                                hold off;

axis([0 t_sol(end) 0 agents]);          xlabel('time');         ylabel('# Agents');         
legend('R stoc','Rp stoc','Rpp stoc','Rppp stoc','R deter','Rp deter','Rpp deter','Rppp deter');
