...
for n=1:s
    ...
    for w=1:reps
        ...
        ABK_Rp(w,n) = mean(Trp(w,end-25*dt^-1:end));
...
for n=1:s
    ...
    Rp_ss(n) = agents * S / ( k_r/k_f + S );
    ssr(n) = k_r * Rp_ss(n);
    % ssr: Steady-state rate of forward and back reactions
    
    dRpdt_synt(n,:) = k_f * S .* (agents - Rp_array);  
...
plot(S_array,Rp_ss,S_array,avg_Rp,'.r');
...
%% Notes
% Correctly predicts hyperbolic signal-response curve.