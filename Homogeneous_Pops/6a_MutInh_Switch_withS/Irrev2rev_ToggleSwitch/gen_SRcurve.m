...
%     fprintf(1,'\nFixed Points:\t\tEigenvalues\n');
    ...
    if isempty(A_ss_int) == 0
        if size(A_ss_int,1) == 2
            A_ss_int = [A_ss_int' NaN];
            B_ss_int = [B_ss_int' NaN];
        end
        A_ss(n,:) = A_ss_int(:);
        B_ss(n,:) = B_ss_int(:);
    else
...
% Sort numerical values of each B_ss row 
for j=1:size(B_ss,1)
    B_ss(j,:) = BubbleSort(B_ss(j,:));
end

% In some cases it is necessary to move the monostable values to 
% either terminal column to ensure the SR curve is drawn right.
%% Plot
...
for h=1:size(B_ss,2)
    if mod(h,2) == 0
        p(h) = plot(S_array,B_ss(:,h),'--','LineWidth',2,...
