...
k_da = 0.005;                       % degradation rate constant (1st order) for A
k_db = 0.01;                        % degradation rate constant (1st order) for B
% Populations producing half-maximal regulatory effect
K_b = 50;                       K_a =  50;
K_s = 50;

S_array = 0:2:100;                  % Number of S molecules to do simulation for
...
for a=1:reps
    ...
    for s=1:size(S_array,2)
        ...
        agents = 400;                       % disp(['Agents = ' num2str(agents)]);
        Ta(1) = Ao;                         Tb(1) = Bo;			
        % ************************************************************
...
SR = load('SR_curve.mat','S_array','B_ss');
...
p_ABK = errorbar(S_array(1:2:end),avg_Bss(1:2:end),sdev_Bss(1:2:end),...
    'or','DisplayName','<N_R^*>_{sim}');                        % Using Matlab built-in function
...
axis([S_array(1) S_array(end) 0 113]);                            
...
plot(10*ones(1,100),1:100,'--','Color',[1 0.69 0.39]);
plot(74.1*ones(1,100),1:100,'--','Color',[1 0.69 0.39]);        
plot(S_array,ON,'ob');
