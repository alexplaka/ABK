...
for i=1:size(R_ss,2)-1
    ...
    plot(time_part,R_ss(i)*ones(1,size(time_part,2)),'--','Color',[1 0.75 0.2],'LineWidth',2);
end
time_part = totalTime*(i)/5:totalTime;   % Plot R_ss for last value of S - Extended time interval
plot(time_part,R_ss(i+1)*ones(1,size(time_part,2)),'--','Color',[1 0.75 0.2],'LineWidth',2);
