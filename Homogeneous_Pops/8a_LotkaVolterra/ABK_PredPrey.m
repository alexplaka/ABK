...
maxTime = 30;                     % Maximum Simulation time (sec)
...
while t*dt <= maxTime && Tr(t) < 0.9*agents
...
%% Spectral Analysis for DE trajectory
fs = 1/dt;                                          % sampling frequency
t_osc = 0:finaltime/1000:finaltime;                      
R_osc = y_sol(:,1)';                     F_osc = y_sol(:,2)';
...
%     axis([0.5*f_th 2*f_th 0 12*q]);
    xlim([10*f_th 500*f_th]);
    ylim([0 6*Zn(q,find(abs(f-f_th)<=0.0005 & abs(f-f_th)>=0.00005,1))]);
...
[Rm,Fm] = meshgrid(0:ul/10:ul);        % Mesh-grid values for constructing state space
...
p_ncR = plot([0 ul],[R_nc R_nc],'Color',[1 0.3 0.2],...
...
set(gca,'XMinorTick','on','YMinorTick','on','Box','off');
axis([0 ul 0 ul]);          set(gca,'DataAspectRatio',[1 1 1]);            
