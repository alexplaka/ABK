...
cd DDE_files;
DDEsol = dde23(@negFb_delaydif,lag,@negFb2_dde_hist,[to , totalTime]);
...
trial = 3;                         % Choose simulation run/trial (trial <= reps)
...
leg0 = legend('ABK N_R(t)','ABK N_{Xp}(t)','DDE N_R(t)','DDE N_{Xp}(t)','Location','Best');
...
p_avgX = plot(time(1:20:end),avg_X(1:20:end),'b','LineWidth',2,'DisplayName','ABK <N_R(t)>');
plot(time(1:20:end),avg_X(1:20:end)+sdev_X(1:20:end),'LineStyle','--','Color',[0.8 0.8 0.8]);
plot(time(1:20:end),avg_X(1:20:end)-sdev_X(1:20:end),'LineStyle','--','Color',[0.8 0.8 0.8]);

p_avgRp = plot(time(1:20:end),avg_Rp(1:20:end),'r','LineWidth',2,'DisplayName','ABK <N_{Xp}(t)>');
% plot(time(1:20:end),avg_Rp(1:20:end)+sdev_Rp(1:20:end),'LineStyle','--','Color',[0.8 0.8 0.8]);
% plot(time(1:20:end),avg_Rp(1:20:end)-sdev_Rp(1:20:end),'LineStyle','--','Color',[0.8 0.8 0.8]);

p_ddeX = plot(t_sol,y_sol(:,1),'--','Color',[0.15 1 0.75],'LineWidth',2,'DisplayName','DDE  N_R(t)'); 
p_ddeRp = plot(t_sol,y_sol(:,2),'--','Color',[0.75 0 1],'LineWidth',2,'DisplayName','DDE  N_{Xp}(t)');  
...
% t_osc = time;                             % choose time interval with oscillations               
% % X_osc = avg_X;                          Rp_osc = avg_Rp;
% X_osc = X_all(trial,:);                 Rp_osc = Rp_all(trial,:);
% 
% % figure;     plot(t_osc,X_osc,t_osc,Rp_osc);
% % xlabel = ('time (sec)');        ylabel('#');
% 
% Osc = [X_osc; Rp_osc];
% f_th = imag(lambdas(1)) / (2*pi);                   % Theoretical oscillation frequency (Hz)
...
% f = fs / n * (0:n/2);                   species = {'X' , 'Rp'};
% 
% for b=1:2
%     Z(b,:) = fft(Osc(b,:),n);
%     Zn(b,:) = abs(Z(b,:)/n);
%     subplot(2,1,b);                     title(species{b});                      hold on;
%     plot(f,Zn(b,1:n/2+1)); 
%     tempy = [0 25 50];                      tempf = [f_th f_th f_th];           
%     plot(tempf,tempy,'--','Color',[1 0.75 0.25]); 
%     axis([0.5*f_th 1.5*f_th 0 0.4]);
