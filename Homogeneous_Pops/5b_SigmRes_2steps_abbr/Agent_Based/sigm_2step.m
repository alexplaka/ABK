...
% syms R Rp positive;
% dR_sym =  -k_f * R * S*(S-1) + k_r * Rp;
% dRp_sym =  k_f * R * S*(S-1) - k_r * Rp;
% 
% % Nullclines
...
Rp_ss = agents * S*(S-1) / ( k_r/k_f + S*(S-1) );
%% Construct Rate curve
Rp_array = 0:agents;

dRpdt_degr = k_r .* Rp_array;
dRpdt_synt = k_f * S*(S-1) .* (agents - Rp_array);  
...
% legend('R nullcline','R_p nullcline','Direction field','Deterministic','ABM stochastic');
