...
for n=1:s
    ...
    Rp_ss(n) = agents * S^2 / ( k_r/k_f + S^2 );
    ssr(n) = k_r * Rp_ss(n);
...
p_de = plot(S_array,Rp_ss,'DisplayName','DE Dual-Step');
p_abk = plot(S_array,avg_Rp,'.r','DisplayName','ABK Dual-Step');
plot(S_array,avg_Rp+std_Rp,'--','Color',[0.8 0.8 0.8]);
plot(S_array,avg_Rp-std_Rp,'--','Color',[0.8 0.8 0.8]);
...
SST = sum((avg_Rp' - mean(avg_Rp)).^2);     % Total sum of squares for simulation data (for A)
SSR = sum((avg_Rp' - Rp_ss').^2);           % sum of square residuals (sim data vs DE predictions)
...
% Compare to motif with same parameters from hyperbolic SR.